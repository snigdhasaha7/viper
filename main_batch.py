...
def run_program(parameters, queues_in_, input_type_, retrying=False):
    from image_patch import ImagePatch, llm_query, best_image_match, distance, bool_to_yesno, process_guesses, formatting_answer
    ...
    code_header = f'def execute_command_{sample_id}(' \
                  ...
                  'formatting_answer, llm_query, bool_to_yesno, distance, best_image_match, process_guesses):\n' \
                  f'    # Answer is:'
    ...
    formatting_answer_partial = partial(formatting_answer, queues=queues)
    ...
    try:
        result = globals()[f'execute_command_{sample_id}'](
            ...
            formatting_answer_partial, llm_query_partial, bool_to_yesno, distance, best_image_match, process_guesses_partial)
    except Exception as e:
        ...
        try:
            ...
            for i, batch in tqdm(enumerate(dataloader), total=n_batches):
                ...
                if not config.use_cached_codex:
                    codes, _ = codex(prompt=batch['query'], base_prompt=base_prompt, input_type=input_type,
                ...
                if config.execute_code:
                    if not config.multiprocessing:
                        ...
                        results = []
                        sample_ids = [batch['sample_id'][0]] * len(codes) 
                        imgs = [batch['image'][0]] * len(codes) 
                        answers = [batch['possible_answers'][0]] * len(codes) 
                        queries = [batch['query'][0]] * len(codes)
                        for c, sample_id, img, possible_answers, query in \
                                zip(codes, sample_ids, imgs, answers, queries):
                ...
                all_codes.append([r[1] for r in results])
    ...
    if config.save:
        ...
        if not config.save_new_results:
            ...
            if len(existing_files) == 0:
                filename_csv = 'results_0.csv'
                filename_json = 'results_0.json'
            else:
                next_index = str(max([int(ef.stem.split('_')[-1]) for ef in existing_files if
                                                                           str.isnumeric(ef.stem.split('_')[-1])]) + 1)
                filename_csv = f'results_{next_index}.csv'
                filename_json = f'results_{next_index}.json'
        print('Saving results to', filename_csv, 'and', filename_json)
        ...
        df.to_csv(results_dir / filename_csv, header=True, index=False, encoding='utf-8')
        
        # Save to JSON
        results_list = df.to_dict(orient='records')
        with open(results_dir / filename_json, 'w', encoding='utf-8') as f:
